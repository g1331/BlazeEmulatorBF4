	...
	if func == '0064':
		...
		for i in range(len(pids)):
			...
			for x in range(len(content[i])):
				for pStat in pStats:
					if content[i][x][0][:-2] == pStat[0] or content[i][x][0] == pStat[0]:
						stat = float(content[i][x][1]) + float(pStat[1])
						pStat[1] = str(stat)

			#f = open("Users/"+name+"/userstats.txt", 'w')
			statsToWrite = ""
			for pStat_ in pStats:
				#f.write(pStats[y][0]+"="+pStats[y][1]+"\n")
				statsToWrite = statsToWrite + pStat_[0] + "=" + pStat_[1] + "\n"
		...
		print(f"[GMRPT] ERROR! UNKNOWN FUNC {func}")
